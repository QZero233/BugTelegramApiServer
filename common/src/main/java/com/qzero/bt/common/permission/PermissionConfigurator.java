...
public class PermissionConfigurator {
    ...
    @PermissionConfig(value = PermissionNameList.PERMISSION_MANAGE_USER)
    private boolean manageUser(TokenEntity token){
        UserInfoEntity userInfoEntity=userInfoDao.getUserInfo(token.getOwnerUserName());
        return userInfoEntity.getGroupLevel()>=UserInfoEntity.GROUP_ADMIN;
    }
